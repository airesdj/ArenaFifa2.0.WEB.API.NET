...
DELIMITER $$
DROP PROCEDURE IF EXISTS `spDeleteAllSpoolerBySigla` $$
CREATE PROCEDURE `spDeleteAllSpoolerBySigla`(pSiglaProcess VARCHAR(30))
begin
	DELETE FROM TB_PROCESSOS_EMAIL_DETALHE 
	WHERE ID_PROCESSO IN (SELECT ID_PROCESSO FROM TB_SPOOLER_PROCESSOS_EMAIL WHERE SL_PROCESSO = pSiglaProcess);
	
	DELETE FROM TB_SPOOLER_PROCESSOS_EMAIL WHERE SL_PROCESSO = pSiglaProcess;
End$$
DELIMITER ;
