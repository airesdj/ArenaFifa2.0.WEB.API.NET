...
Begin
	...
	call `arenafifadb`.`spCalculateAllFasesEndOfTemporadaPRO`(pIdTemp, _idNewTemp);
...
Begin
	...
	get_tabela: LOOP
		...
		IF _count = 0 THEN
			call `arenafifadb`.`spAddHistoricoTemporadaPRO`(pIdTemp, _idUsuHome, NULL);
		END IF;
		...
		IF _count = 0 THEN
			call `arenafifadb`.`spAddHistoricoTemporadaPRO`(pIdTemp, _idUsuAway, NULL);
		END IF;
	...
	call `arenafifadb`.`spCalculateFasePlayoffFUT_PRO`(pIdTemp, _sgCamps, _sgLigas, "PRO");
...
Begin
	...
	get_tabela: LOOP
		...
		call `arenafifadb`.`spCalculatePerformanceTecnicosFUT_PRO`(pIdTemp, _idUsu, pSgLigas, "PRO");
		
		call `arenafifadb`.`spCalculatePerformanceGeralTecnicosFUT_PRO`(pIdTemp, _idUsu, pSgLigas, "PRO");
...
Begin
	...
	get_tabela: LOOP
		...
		call `arenafifadb`.`spAddHistoricoTemporadaPRO`(pIdTemp, _idUsu, 0);
...
Begin
	call `arenafifadb`.`spUpdateHallOfFameLstNegraPRO`(pIdTemp);
