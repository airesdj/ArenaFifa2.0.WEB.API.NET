...
Begin
	...
	CREATE TABLE `TB_TABLETEAM_PROVISION` (
	  ...
	  INDEX (`NU_TIME_HOME`), 
	  INDEX (`NU_TIME_AWAY`), 
	  PRIMARY KEY (`IN_TABELA_GRUPO`, `ID_TIPO_TABELA`, `NU_TIME_HOME`, `NU_TIME_AWAY`)
	) ENGINE=myisam DEFAULT CHARSET=utf8;
...
begin      
	...
	IF _count > 0 THEN
		...
		DELETE FROM TB_TABLETEAM_PROVISION_ONLINE WHERE NU_TIME_HOME >= 0;
		DELETE FROM TB_AUTOMATIC_DRAW_TB_TIME_TEMP WHERE ID_TIME >= 0;
		DELETE FROM TB_TABELA_JOGO WHERE ID_CAMPEONATO = pIdCamp;
		DELETE FROM TB_COMENTARIO_USUARIO WHERE ID_CAMPEONATO = pIdCamp;
		...
		ELSEIF _firstFase = _idFaseClassif THEN
		
			IF _inGrupo = false THEN
...
begin      
	...
	INSERT INTO TB_TABLETEAM_PROVISION_ONLINE SELECT * FROM TB_TABLETEAM_PROVISION WHERE IN_TABELA_GRUPO = 1 AND ID_TIPO_TABELA = pQtMaxTimes;
	...
	get_tabela: LOOP
		...
		WHERE NU_TIME_HOME = _idDraw;

		UPDATE TB_TABLETEAM_PROVISION_ONLINE
		SET NU_TIME_AWAY = _idTime
		WHERE NU_TIME_AWAY = _idDraw;
...
begin      
	...
	INSERT INTO TB_TABLETEAM_PROVISION_ONLINE SELECT * FROM TB_TABLETEAM_PROVISION WHERE IN_TABELA_GRUPO = 0 AND ID_TIPO_TABELA = pQtMaxTimes;
...
begin      
	...
	get_tabela: LOOP
		...
		call `arenafifadb`.`spAddTabelaJogo`(pIdCamp, pFirstFase, _dtInicio, pIntervalDias, _nuRodada, _idTimeHome, _idTimeAway, 0, false);
