...
begin      
   ...
   select C.*, T.NM_Temporada, fcGetTypeModelOfCampeonato(C.SG_TIPO_CAMPEONATO) as MODE_TYPE from TB_CAMPEONATO C, TB_TEMPORADA T 
...
begin      
   select C.*, T.NM_Temporada, U.NM_Usuario, U.PSN_ID, U.DS_EMAIL, U2.NM_Usuario as NM_Usuario2, U2.PSN_ID as PSN_ID2, U2.DS_EMAIL as DS_EMAIL2, TI.DS_TIPO_CAMPEONATO,
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `spGetAllCampeonatosActiveForCalendarView` $$
CREATE PROCEDURE `spGetAllCampeonatosActiveForCalendarView`(pType VARCHAR(3))
begin
	DECLARE _ID_FASE_3o_LUGAR INTEGER DEFAULT 6;
	DECLARE _ID_FASE_CLASSIFICACAO INTEGER DEFAULT 0;
	
   SELECT C.*, 
   (SELECT T.DT_TABELA_FIM_JOGO FROM TB_TABELA_JOGO T WHERE T.ID_Campeonato = C.ID_CAMPEONATO AND ID_FASE = _ID_FASE_CLASSIFICACAO ORDER BY IN_NUMERO_RODADA DESC LIMIT 1) as DT_TABELA_FIM_JOGO,
   (SELECT count(1) FROM TB_FASE_CAMPEONATO F WHERE F.ID_CAMPEONATO = C.ID_CAMPEONATO AND (F.ID_Fase > _ID_FASE_CLASSIFICACAO Or F.ID_Fase < _ID_FASE_CLASSIFICACAO) AND F.ID_Fase < _ID_FASE_3o_LUGAR) as TOTAL_FASE_CAMPEONATO,
   (SELECT count(1) FROM TB_FASE_CAMPEONATO F2 WHERE F2.ID_CAMPEONATO = C.ID_CAMPEONATO AND F2.ID_Fase = _ID_FASE_CLASSIFICACAO) as TEM_FASE_CLASSIFICACAO
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `spGetDetailsCampeonatoForLineUpView` $$
CREATE PROCEDURE `spGetDetailsCampeonatoForLineUpView`(pIdCamp INTEGER)
begin
	DECLARE _ID_FASE_CLASSIFICACAO INTEGER DEFAULT 0;
	
   SELECT C.*, 
   (SELECT COUNT(1) FROM TB_FASE_CAMPEONATO WHERE ID_CAMPEONATO = pIdCamp AND ID_FASE > _ID_FASE_CLASSIFICACAO) as TOTAL_FASES_PLAYOFF,
   X.ID_FASE as FIRST_ID_FASE_PLAYOFF, F.NM_FASE as NM_FIRST_FASE_PLAYOFF,
   (SELECT ID_FASE FROM TB_FASE_CAMPEONATO WHERE ID_CAMPEONATO = pIdCamp ORDER BY ID_FASE LIMIT 1) as FIRST_ID_FASE,
   X2.ID_FASE as ID_FASE_IN_PROGRESS, F2.NM_FASE as NM_FASE_IN_PROGRESS,
   (SELECT count(1) FROM TB_FASE_CAMPEONATO WHERE ID_CAMPEONATO = pIdCamp AND ID_Fase = _ID_FASE_CLASSIFICACAO) as HAS_FASE_0
   FROM TB_CAMPEONATO C, (SELECT ID_CAMPEONATO, ID_FASE FROM TB_FASE_CAMPEONATO WHERE ID_CAMPEONATO = pIdCamp AND ID_FASE > _ID_FASE_CLASSIFICACAO ORDER BY ID_FASE LIMIT 1) X, 
	    TB_FASE F, TB_FASE F2, (SELECT ID_CAMPEONATO, ID_FASE FROM TB_TABELA_JOGO WHERE ID_CAMPEONATO = pIdCamp AND QT_GOLS_TIME_CASA IS NULL ORDER BY ID_FASE LIMIT 1) X2
   WHERE C.ID_CAMPEONATO = pIdCamp
   AND C.ID_CAMPEONATO = X.ID_CAMPEONATO
   AND X.ID_FASE = F.ID_FASE
   AND C.ID_CAMPEONATO = X2.ID_CAMPEONATO
