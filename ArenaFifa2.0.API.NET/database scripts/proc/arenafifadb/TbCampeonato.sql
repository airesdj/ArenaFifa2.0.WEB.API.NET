...
begin      
   select C.*, (C.DT_INICIO - CURDATE()) as DT_CAMPEONATO_INICIADO, T.NM_Temporada from TB_CAMPEONATO C, TB_TEMPORADA T 
...
CREATE PROCEDURE `spGetAllCampeonatosNoFilterCRUD`(pFiltroAtivo TINYINT)
begin      
   ...
   and  C.IN_CAMPEONATO_ATIVO = pFiltroAtivo
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `spGetAllCampeonatosWithFilterCRUD` $$
CREATE PROCEDURE `spGetAllCampeonatosWithFilterCRUD`(pFiltroAtivo TINYINT, pFilter VARCHAR(20))
begin      
   select C.*, T.NM_Temporada, DATE_FORMAT(C.DT_INICIO,'%d/%m/%Y') as dt_inicio_formatada, DATE_FORMAT(C.DT_SORTEIO,'%d/%m/%Y') as dt_sorteio_formatada, U1.PSN_ID as PSN1,
   (select U2.PSN_ID from TB_USUARIO U2 where U2.ID_USUARIO = C.ID_USUARIO_2oMODERADOR) as PSN2
   from TB_CAMPEONATO C, TB_TEMPORADA T, TB_USUARIO U1
   where (C.NM_CAMPEONATO like CONCAT('%',pFilter,'%') or T.NM_Temporada like CONCAT('%',pFilter,'%'))
   and C.ID_Temporada = T.ID_Temporada
   and  C.ID_USUARIO_MODERADOR = U1.ID_USUARIO
   and  C.IN_CAMPEONATO_ATIVO = pFiltroAtivo
   order by C.ID_TEMPORADA DESC, C.IN_CAMPEONATO_ATIVO, C.ID_CAMPEONATO;      
...
begin      
   select C.*, T.NM_Temporada, U.NM_Usuario, U.PSN_ID, U2.NM_Usuario as NM_Usuario2, U2.PSN_ID as PSN_ID2,
   (select count(1) from TB_TABELA_JOGO where ID_CAMPEONATO = pIdCamp and T_EFETIVACAO_JOGO is not null) as inInicioCampeonato,
   (select ID_Fase from TB_FASE_CAMPEONATO where ID_CAMPEONATO = pIdCamp order by ID_FASE limit 1) as idPrimFaseCampeonato
   from TB_CAMPEONATO C, TB_TEMPORADA T, TB_USUARIO U, TB_USUARIO U2
