...
DELIMITER $$
DROP PROCEDURE IF EXISTS `spAddLoadComentarioUsuarioOfCampeonatoByFase` $$
CREATE PROCEDURE `spAddLoadComentarioUsuarioOfCampeonatoByFase`(pIdCamp INTEGER, pIdFase INTEGER)
Begin
	INSERT INTO TB_COMENTARIO_USUARIO
		SELECT J.ID_TABELA_JOGO, J.ID_CAMPEONATO, U1.ID_USUARIO
		from TB_TABELA_JOGO J, TB_TIME T1, TB_USUARIO_TIME X1, TB_USUARIO U1
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `spDeleteComentarioUsuarioOfTabelaJogoByFase` $$
CREATE PROCEDURE `spDeleteComentarioUsuarioOfTabelaJogoByFase`(pIdCamp INTEGER, pIdFase INTEGER)
Begin
    delete from `TB_COMENTARIO_USUARIO` 
	where ID_CAMPEONATO = pIdCamp
	and ID_TABELA_JOGO in (select J.ID_TABELA_JOGO from TB_TABELA_JOGO J where J.ID_CAMPEONATO = pIdCamp AND J.ID_FASE = pIdFase);
End$$
DELIMITER ;