...
Begin
	...
	IF _count = 0 THEN
		...
		call `arenafifadb_staging`.`spTransferDataFromDBToBKP`();
...
Begin
	...
	get_tabela: LOOP
		...
		call `arenafifadb_staging`.`spAddUsuarioTimev2`(_idCamp, _idUsu, _idTime, _inOrdem);
...
Begin
	...
	get_tabela: LOOP
		...
		ELSEIF _TotLstNegra <= pQtLimitMaxLstNegra THEN
			...
			call `arenafifadb_staging`.`spAddHistoricoTemporada`(pIdTemp, _idUsu, pCodAcessoTapetao);
...
Begin
	...
	get_tabela: LOOP
		...
		call `arenafifadb_staging`.`spAddHistoricoTemporada`(pIdTemp, _IdUsu1, pCodAcesso);
		...
		call `arenafifadb_staging`.`spAddHistoricoTemporada`(pIdTemp, _IdUsu2, pCodAcesso);
...
Begin
	...
	get_tabela: LOOP
		...
		ELSEIF _TotLstNegra <= pQtLimitMaxLstNegra THEN
			...
			call `arenafifadb_staging`.`spAddHistoricoTemporada`(pIdTemp, _idUsu, pCodAcessoConvite);
